...
async def proxy_server(reader, writer, backend, description):
    async with SEMAPHORE:
        ...
        logger.info(f"Семафор захвачен (свободно: {SEMAPHORE._value})")
        try:
            while True:
                ...
                writer.write(data)
                await asyncio.wait_for(writer.drain(), timeout=100)
    ...
    logger.info(f"Семафор освобожден (свободно: {SEMAPHORE._value})")
